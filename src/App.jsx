...
function App() {
  ...
  const redirect = async () => {
    ...
    if (link) await fetch(`${base_api_url}/redirect/${link}`);
  }
      ...
      <div className='container'>
        ...
        {shortLinks.length > 0 && <div className='links-container'>
          {shortLinks.map((link, key) => <div key={key} className='link'>
            ...
            <a href={`${base_api_url}/redirect/${link.shortUrl}`} target='_blank' rel="noreferrer">https://shortli.netlify.app/{link.shortUrl}</a>
          </div>)}
        </div>}
      </div>
    </>
